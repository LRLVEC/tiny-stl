...
//sort
template <class T>
inline bool judgeUp(T *const a, int p, int q)
{
...
template <class T>
inline void maxHeap(T *const a, int p, int q)
{
	T l((p << 1) + 1);
	T r((p + 1) << 1);
	if (a[p] < a[l])
...
template <class T>
inline void maxTopHeap(T *const a, int q)
{
	if (1 < q && *a < a[1])
	{
		if (2 < q && a[1] < a[2])
...
template <class T>
inline void buildMaxHeap(T *const a, int q)
{
	for (int c((q >> 1) - 1); c; --c)
		maxHeap(a, c, q);
	maxTopHeap(a, q);
...
template <class T>
inline void heapSort(T *const a, int q)
{
	for (int c((q >> 1) - 1); c; --c)
		maxHeap(a, c, q);
	maxTopHeap(a, q);
...
template <class T>
inline void qsort(T *const a, int p, int q)
{
	if (p + 1 < q)
	{
		T const &k(a[p]);
...
struct Interval
{
	...
	Interval(R const &_a, S const &_b)
		: a(_a),
		  b(_b)
	{
...
template <class T>
struct IntervalSet : Vector<Interval<T>>
{
	...
	template <class R>
	IntervalSet(Vector<R> const &a, T const &b, bool withBorder) : Vector<Interval<T>>()
	{
		static_assert(NumType<R>::value == true, "Non-numeric type not supported yet!");
		B::malloc(a.length);
	...
	IntervalSet<T> operator^(Interval<T> const &s) const
	{
		...
		IntervalSet<T> tp(*this);
		tp.simplify();
		if (s.a > tp.end().b || s.b < tp.data[0].a)
			return IntervalSet<T>();
...
//Interval
template <class T>
IntervalSet<T> Interval<T>::operator^(IntervalSet<T> const &s) const
{
	return s ^ (*this);
}
...
//_Vector.h
template <class T>
inline Vector<T> Vector<T>::truncate(Interval<int> const &s) const
{
	Interval<int> itvl(0, length - 1);
	itvl ^= s;
	if (itvl.valid())
		return truncate(itvl.a, itvl.b - itvl.a + 1);
	return Vector<T>();
}
template <class T>
inline Vector<T> Vector<T>::truncate(IntervalSet<int> const &s) const
{
...
template <class T>
inline Vector<T> &Vector<T>::truncateSelf(Interval<int> const &s)
{
	Interval<int> itvl(0, length - 1);
	itvl ^= s;
	if (itvl.valid())
		return truncateSelf(itvl.a, itvl.b - itvl.a + 1);
	this->~Vector();
	return *this;
}
template <class T>
inline Vector<T> &Vector<T>::truncateSelf(IntervalSet<int> const &s)
{
	IntervalSet<int> tp(s ^ Interval<int>(0, length - 1));
	length = tp.area(true);
	if (!length)
